...
function WatchLater() {
  ...
  useEffect(() => {
    ...
    const results = (watchLater || []).filter((m) => (m.title || "").toLowerCase().includes(q));
  ...
  if (watchLater && watchLater.length > 0) {
    return (
      <div className="favs">
        ...
        <form onSubmit={(e) => e.preventDefault()} className="search-form" style={{ maxWidth: 700, margin: "0 auto 1.5rem" }}>
          <input
            type="text"
          ...
          />
          <button type="submit" className="search-btn icon-only" aria-label="Search" title="Search">
